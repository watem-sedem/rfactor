...
def test_rain_energy_per_unit_depth(rain, energy):
    ...
    assert energy == approx(rain_energy_verstraeten(rain), nan_ok=True, abs=1e-5)
...
def test_compute_erosivity_wrong_df():
    ...
    with pytest.raises(RFactorInputError) as excinfo:
        _compute_erosivity(df, rain_energy_verstraeten, maximum_intensity)
    assert "contain data of a single year." in str(excinfo.value)


def test_compute_0_or_NULL_rain_mm_error(dummy_rain, recwarn):
    ...
    # test warning
    with pytest.raises(RFactorInputError) as excinfo:
        compute_erosivity(df, rain_energy_verstraeten, maximum_intensity)
...
def test_apply_rfactor(rain_benchmark_closure):
    ...
    erosivity_support_func = _compute_erosivity(
        rain, rain_energy_verstraeten, maximum_intensity
    )
...
def test_rfactor_benchmark_single_year(
    ...
    erosivity = compute_erosivity(rain, rain_energy_verstraeten, intensity_method)
